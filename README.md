...
- Manual-order attachments are uploaded to Shopify Files and linked from the production job and internal order-copy email. Attachments must be 20 MB or smaller. The inventory app installation and `SHOPIFY_ADMIN_ACCESS_TOKEN` must include Shopify's `write_files` Admin API access scope.
