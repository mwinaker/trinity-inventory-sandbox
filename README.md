...
- Manual-order attachments are uploaded to Shopify Files, linked from the production job, and included as a prominent download link in the standard internal order-copy email sent to Jeremy and the other required staff recipients. When Shopify reports the order paid, including a completed draft that Shopify creates already paid, the order webhook sends Jeremy a second Shopify email with the same internal download link. Submission and paid delivery events are recorded on the production job and copied to the order's `trinity.internal_attachment_notifications` metafield so duplicate webhooks do not resend the message. This works through Shopify without Resend and does not expose the file in the customer order payload. Attachments must be 20 MB or smaller. The inventory app installation and `SHOPIFY_ADMIN_ACCESS_TOKEN` must include Shopify's `write_files` Admin API access scope.
