...
- Manual-order attachments are uploaded to Shopify Files, linked from the production job, and included as a prominent download link in the standard internal order-copy email sent to Jeremy and the other required staff recipients. Shopify Flow sends Jeremy the paid-order update: use its **Order paid** trigger and **Send internal email** action, with a condition that the `trinity.internal_attachment` order metafield is present and the message link `{{ order.metafields.trinity.internal_attachment.value.downloadUrl }}`. Keep the companion **Order created** workflow for completed drafts that are created already paid, after a short wait so the app can copy the internal attachment metafield to the completed order. The app records the paid Flow email request on the production job and the order's `trinity.internal_attachment_notifications` metafield; Flow's run history is the delivery record. This works through Shopify without Resend and does not expose the file in the customer order payload. Attachments must be 20 MB or smaller. The inventory app installation and `SHOPIFY_ADMIN_ACCESS_TOKEN` must include Shopify's `write_files` Admin API access scope.
